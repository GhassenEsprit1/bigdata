...
@Slf4j
@NoArgsConstructor
public class VilleLineRecordReader extends RecordReader<VilleLongWritable, VilleText> {
    ...
    private VilleLongWritable key;
    private VilleText value;
    private boolean isCompressedInput;
    private Decompressor decompressor;
    private byte[] recordDelimiterBytes;


    public VilleLineRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
        ...
        if (this.start != 0L) {
            this.start += (long)this.in.readLine(new VilleText(), 0, this.maxBytesToConsume(this.start));
        }
    ...
    public boolean nextKeyValue() throws IOException {
        if (this.key == null) {
            this.key = new VilleLongWritable();
        }

        this.key.set(this.pos);
        if (this.value == null) {
            this.value = new VilleText();
        }
    ...
    public VilleLongWritable getCurrentKey() {
        return this.key;
    }

    public VilleText getCurrentValue() {
        return this.value;
    }
